...
async fn random_entry(_req: tide::Request<()>) -> AppResponse {
    match model::random_entry() {
        ...
        Err(_) => abort(tide::StatusCode::InternalServerError, "Failed to retrieve random entry."),
    }
}

async fn entry(req: tide::Request<()>) -> AppResponse {
    match req.param("slug") {
        Ok(slug) => match model::get_entry(slug) {
            Ok(e) => Ok(render_entry(e)),
            Err(_) => abort(tide::StatusCode::NotFound, "No such entry."),
        },
        Err(_) => abort(tide::StatusCode::BadRequest, "usage: /entry/<entry slug>"),
    }
}
